...
# Written by Genadi Naydenov <genadi.naydenov@york.ac.uk> University of York (2016)
#
...
def main(argv = None):
    ...
    #     supercell_size = nx*ny*nz
      elif 'Force Constant matrix' in line:
         ifc_present = True
    if ifc_present == False:
         raise ValueError("Force Coonstant matrix not found in .castep."
	"\nPlease add 'phonon_write_force_constants: true' to the .param file \nand perform a continuation run.")
    ...
    for index, line in enumerate(castep_data):
         ...
         for z in range(supercell_size):
           ...
           if supercell in line:
              ...
              for n in range(ions):
                 for i in range(3): 
                    # CASTEP data is stored in 6 columns with ceil(atoms/2) rows.
                    ...
                    for f in range(l_castep):                                             
                        ...
                        if len(line_split)==8:
                           line_data+=(float(line_split[j]) for j in range(2, 8))
                        elif len(line_split)<8 and len(line_split)>3:
                           line_data+=(float(line_split[j]) for j in range(0, 6))
                        elif len(line_split)<4:
                           line_data+=(float(line_split[j]) for j in range(0, 3))
                     
                    supercell_3nx3n_matrix += [line_data]
                    for m in range(ions):
                        ifc_data[supercell][ions*n+m][i]=[float(supercell_3nx3n_matrix[3*n+i][j])
								     for j in range(m*3, 3*m+3)]
...
# constructured by increasing x first, then y and then z.
    ...
    ion_reset=0 
    indeces_array = []
    for every_atom in range(0,ions*nx*ny*nz):
